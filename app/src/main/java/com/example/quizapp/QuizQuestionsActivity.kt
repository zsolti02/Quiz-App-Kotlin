...
class QuizQuestionsActivity : AppCompatActivity() {
        ...
        btnSubmit?.setOnClickListener {
            if (!isAnswerChecked) {
                ...
                if (!anyAnswerIsChecked) {
                    Toast.makeText(this, "Kérlek válassz egyet!", Toast.LENGTH_SHORT).show()
                } else {
                    ...
                    btnSubmit?.text = if (currentQuestionIndex == questionsList.size - 1) "BEFEJEZEM" else "KÖVETKEZŐ KÉRDÉS"
    ...
    private fun updateQuestion() {
        ...
        btnSubmit?.text = if (currentQuestionIndex == questionsList.size - 1) "BEFEJEZEM" else "ELLENŐRZÉS"
    }
